...
create table baralho_cartadojogo (
    ...
    CONSTRAINT FK_baralho_id FOREIGN KEY (baralho_id) REFERENCES baralho (id)
);

create table baralho_cartainicio (
    baralho_id uuid not null,
    cartainicio_id uuid not null,
    CONSTRAINT FK_cartainicio_id FOREIGN KEY (cartainicio_id) REFERENCES carta_inicio (id),
    CONSTRAINT FK_baralho_id FOREIGN KEY (baralho_id) REFERENCES baralho (id)
);
...
create table baralho_cartaobjetivo (
    ...
    CONSTRAINT FK_cartaobjetivo_id FOREIGN KEY (cartaobjetivo_id) REFERENCES carta_objetivo (id),
    CONSTRAINT FK_baralho_id FOREIGN KEY (baralho_id) REFERENCES baralho (id)
);
...
create table jogador_cartadojogo (
    ...
    CONSTRAINT FK_car_id FOREIGN KEY (car_id) REFERENCES carta_do_jogo (id),
    CONSTRAINT FK_jog_id FOREIGN KEY (jog_id) REFERENCES jogador (id)
);

create table jogador_cartaobjetivo (
    jog_id uuid not null,
    car_id uuid not null,
    CONSTRAINT FK_car_id FOREIGN KEY (car_id) REFERENCES carta_objetivo (id),
    CONSTRAINT FK_jog_id FOREIGN KEY (jog_id) REFERENCES jogador (id)
);
